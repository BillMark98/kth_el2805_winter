...
# Load model
try:
    critic_nn = torch.load('neural-network-3-critic.pth')
    actor_nn = torch.load('neural-network-3-actor.pth')
...
v_values = critic_nn(states_tensor)

max_q = v_values.data.numpy().reshape(X_coords.shape)
...
plt.title("mean action for direction")
...
#     v_values = model(torch.tensor([state]))
