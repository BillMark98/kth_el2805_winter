...
from PPO_agent import PPO_agent, RandomAgent
...
def simulate(env, agent, episodes, agentName = "PPO"):
    ''' simulate '''
...
# create two agents
ppo_agent = PPO_agent(n_actions=2,loadPrev=True)
random_agent = RandomAgent(n_actions=2)
ppo_episode_reward_list = simulate(env, ppo_agent, N_EPISODES, "PPO")
random_episode_reward_list = simulate(env, random_agent, N_EPISODES, "Random")

# plot the rewards
# save Name
saveName = "Random_PPO_rewards.png"
plt.plot(list(range(1,len(ppo_episode_reward_list) + 1)), ppo_episode_reward_list, label = 'ppo')
plt.plot(list(range(1,len(random_episode_reward_list) + 1)), random_episode_reward_list, label = 'random')
plt.title("Random vs PPO")
plt.ylabel("reward")
plt.xlabel("episode")
plt.legend()
