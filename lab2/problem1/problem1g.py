...
from DQN_agent import DQN_agent, RandomAgent
...
def simulate(env, agent, episodes, agentName = "DQN"):
    ''' simulate '''
...
# create two agents
dqn_agent = DQN_agent(n_actions=env.action_space.n,loadPrev=True)
random_agent = RandomAgent(n_actions=env.action_space.n)
dqn_episode_reward_list = simulate(env, dqn_agent, N_EPISODES, "DQN")
random_episode_reward_list = simulate(env, random_agent, N_EPISODES, "Random")

# plot the rewards
# save Name
saveName = "Random_DQN_rewards.png"
plt.plot(list(range(1,len(dqn_episode_reward_list) + 1)), dqn_episode_reward_list, label = 'dqn')
